...
namespace KayosMedia.WatchOrNot.Prototype
{
    ...
    public class MovieInfo
    {
        ...
        [NonSerialized] public Texture2D posterTexture;
    ...
    [Serializable]
    public class MovieInfoListWrapper
    {
    ...
    public class SessionInfoManager
    {
        ...
        public static string GetPlaylistMovieFile(string title, string movieCount)
        {
        ...
        public static void Clear()
        {
            ...
            GameManager.OnSessionCleared?.Invoke();
        }

        public static string GetPlaylistMovieCachePath(string title, string movieCount)
        {
            string fileName = GetPlaylistMovieFile(title, movieCount);
            return Path.Combine(Application.persistentDataPath, fileName);
        }

        public static void SaveMoviesToCache(string fileName, List<MovieInfo> movies)
        {
            SaveJson(fileName, new MovieInfoListWrapper { movies = movies });
        }

        public static List<MovieInfo> LoadMoviesFromCache(string fileName)
        {
            var wrapper = LoadJson<MovieInfoListWrapper>(fileName);
            if (wrapper?.movies == null || wrapper.movies.Count == 0)
                return new List<MovieInfo>();
            
            //Restore poster textures from disk cache
            foreach (var movie in wrapper.movies)
            {
                movie.posterTexture = LoadPoster(movie);
            }
