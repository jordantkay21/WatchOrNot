...
namespace KayosMedia.WatchOrNot.Prototype
{
    public static class PlexNetworkingManager
    {
        ...
        public static event Action<bool> OnTokenValidation;
        ...
        public static async Task ValidateStoredTokenAsync()
        {
            ...
            if (string.IsNullOrEmpty(token))
            {
                OnTokenValidation?.Invoke(false);
                Debug.Log($"[PlexNetworkingManager][ValidateStoredTokenAsync] Token returned null.");
                return;
            }
            ...
            try
            {
                ...
                {
                    OnTokenValidation?.Invoke(true);
                    Debug.Log($"[PlexNetworkingManager][ValidateStoredTokenAsync] Token validation succeeded.");
                }
                else
                {
                    OnTokenValidation?.Invoke(false);
                    Debug.Log($"[PlexNetworkingManager][ValidateStoredTokenAsync] Token validation failed.");
                }
            }
            catch (Exception ex)
            {
                OnErrorOccurred?.Invoke(ex.Message);
                Debug.Log($"[PlexNetworkingManager][ValidateStoredTokenAsync] {ex.Message}.");
            }
        }
        #endregion
        ...
        public static async Task<List<MovieInfo>> FetchPlaylistItemsAsync(PlaylistInfo playlist, bool forceRefresh = false)
        {
            List<MovieInfo> movies = new List<MovieInfo>();
            var cacheFile = SessionInfoManager.GetPlaylistMovieFile(playlist.title, playlist.movieCount);
            var cachePath = Path.Combine(Application.persistentDataPath, cacheFile);

            try
            {
                if (!forceRefresh)
                {
                    Debug.Log($"[PlexNetworkingManager][FetchPlaylistItemsAsync] force playlist refresh is false.");
                    //try loading from cache first
                    if (File.Exists(cachePath))
                    {
                        Debug.Log($"[PlexNetworkingManager][FetchPlaylistItemsAsync] cachePath {cachePath} was found" +
                            $".");
                        var cached = SessionInfoManager.LoadMoviesFromCache(cacheFile);
                        
                        if (cached != null && cached.Count > 0)
                        {
                            Debug.Log($"[PlexNetworkingManager][FetchPlaylistItemsAsync] Loaded movies from cache.");
                            SessionInfoManager.SetMovies(cached);
                            return cached;
                        }
                        else
                        {
                            Debug.Log($"[PlexNetworkingManager][FetchPlaylistItemsAsync] Error loading movies from cache. Proceeding to fetch from Plex");
                        }
                    }
                    else
                    {
                        Debug.Log($"[PlexNetworkingManager][FetchPlaylistItemsAsync] cachePath {cachePath} was not found");
                    }
                }

                //No valid cache - proceed to fetch from plex
                string token = SessionInfoManager.LoadToken();
                if (string.IsNullOrEmpty(token)) throw new Exception("No Plex Token Found.");
                ...
                SessionInfoManager.SaveJson(cacheFile, new MovieInfoListWrapper { movies = movies });
                return movies;
            }
            catch (Exception ex)
            {
